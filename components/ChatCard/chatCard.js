...
import { getFirestoreUserByUid } from '../../utility/firestoreUtility'
        ...
        return (
            <TouchableOpacity onPress={() => this.props.action(this.state.userObj)} disabled={this.state.isLoading}>
                <View style={styles.row}>
                    ...
                    <View style={{ flex: 1 }}>
                        <View style={styles.nameContainer}>
                            <Text style={styles.nameTxt} numberOfLines={1} ellipsizeMode="tail">{this.state.userObj.firstName + ' ' + this.state.userObj.lastName}</Text>
                        </View>
                        <View style={styles.msgContainer}>
