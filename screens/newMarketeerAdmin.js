...
import { signup, getCurrentUid } from '../utility/authUtility'
import * as firebase from 'firebase'
import 'firebase/firestore'

export default class NewMarketeerAdmin extends Component {
    constructor(props) {
        super(props)
        this.state = {
            firstName: '', lastName: '', email: '', password: '', phoneNumber: '',
            type: 'Marketeer Admin', account: 'local',
        }
        ...
        if (!this.isFormEmpty()) {
            await this.generatePassword().then(async pass => {
                ...
                let obj = {
                    ...
                    lastName: this.state.lastName, disabled: false, account: 'local', type: "Marketeer Admin", phoneNumber: this.state.phoneNumber
                }
    ...
    render() {
        return (
            <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
                ...
                <InternalHeader flexSize={1} name={'New Marketeer Admin'} action={this.goToDashboard} />
                <View style={{ flex: 10 }}>
                    <View style={{ flex: 4 }}>
